...
class Game(object):
	def __init__(self,keyID=None,soundBool=True,screen=None,res=0):
		...
		self.viewport = pygame.Surface(self.globalscreen.get_size())
	...
	def showStartScreen(self):
		...
		while True:
			...
			self.bannerGameText.draw(self.viewport)
			
			for textBar in self.uiTextBarList:
				textBar.draw(self.viewport)
			...
			self.viewport.blit(levText,levTextRect)
			
			if self.leveldata.levelnamesDict[levelTexts[self.keyID]]==0:
				self.viewport.blit(self.lockedImg,lockedImgRect)
				
			self.banner.draw(self.viewport)
			
			self.viewport.blit(self.pointerImage,self.pointerRect)
			
			#print self.res
			if self.res is 1:
				#print "rguur"
				#ds = pygame.transform.rotozoom(self.viewport,0,2.0)
				#ds = pygame.transform.scale(self.viewport,(640*2,480*2))
				ds = pygame.transform.scale2x(self.viewport)
			else:
				ds = self.viewport
				
			self.globalscreen.blit(ds,(0,0))
	...
	def showCreditsScreen(self):
		...
		while True:
			...
			if notblitted:
				self.viewport.blit(blackSurface,blackSurfaceRect)
				
				line1Text.draw(self.viewport)
				line2Text.draw(self.viewport)
				line3Text.draw(self.viewport)
				line4Text.draw(self.viewport)
	...
	def showInstructionScreen(self):
		...
		while True:
			...
			if notblitted:
				...
				line3Text.draw(self.viewport)
				line4Text.draw(self.viewport)
				line5Text.draw(self.viewport)
				self.viewport.blit(doorClosedImage,doorClosedImageRect)
				self.viewport.blit(doorOpenImage,doorOpenImageRect)
	...
	def showPauseScreen(self):
		...
		while True:
			...
			if notblitted:
				self.viewport.blit(blackSurface,blackSurfaceRect)
				quitFromPauseText.draw(self.viewport)
	...
	def showCompletedScreen(self):
		...
		while True:
			...
			deathText.draw(self.viewport)
			if numOptions==2:
				gotoNextLevelText.draw(self.viewport)
			
			gotoMainMenuText.draw(self.viewport)
	...
	def runGame(self):
		...
		while True:
			...
			self.viewport.blit(self.staticBgImage,self.staticBgRect)
			#screen.blit(self.staticParaImg,self.staticParRect)
			
			self.drawGameObjects(self.viewport)
			self.viewport.blit(textSurf,textRect)
